...
function toItem(login: JellyfinLogin, raw: RawItem): JellyfinItem {
  ...
  return {
    ...
    imageUrl: raw.ImageTags?.Primary
      ? `${login.serverUrl}/Items/${raw.Id}/Images/Primary?maxHeight=400&tag=${raw.ImageTags.Primary}&api_key=${login.accessToken}`
      : undefined,
    // Direct play; the server remuxes only if the container needs it.
    streamUrl: playable
      ? `${login.serverUrl}/Videos/${raw.Id}/stream?static=true&api_key=${login.accessToken}`
      : undefined,
  };
}
