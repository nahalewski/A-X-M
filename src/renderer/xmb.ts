...
/** How many levels' cursor positions to keep. Each entry is a string and a number. */
const LEVEL_MEMORY = 400;
...
export class Xmb {
  ...
  /**
   * Where the cursor sits, remembered per *level* rather than per category, so
   * dropping into a folder and coming back out lands on the folder you opened
   * instead of at the top of the list. The key is the category id plus whatever
   * identifies the level - a folder path, a sub-view name - which also means
   * revisiting a level later restores it, not just backing out of it.
   */
  private selectedIndex = new Map<string, number>();
  /** The level each category is currently showing. */
  private level = new Map<string, string>();
  ...
  /** Moves the cursor to one item of a column, the way a PS3 lands on an inserted disc. */
  selectItem(categoryId: string, itemId: string): boolean {
  ...
  currentSelectedIndex(): number {
    ...
    // Clamped on read: a level can be shorter than when we left it (a deleted file,
    // a shorter search result), and a stale index must not point past the end.
    return Math.min(this.levelIndex(cat.id), Math.max(0, this.currentItems().length - 1));
  }

  /** Composite key for a category's current level. */
  private levelKey(categoryId: string): string {
    return `${categoryId}\u0000${this.level.get(categoryId) ?? ""}`;
  }

  private levelIndex(categoryId: string): number {
    return this.selectedIndex.get(this.levelKey(categoryId)) ?? 0;
  }

  /**
   * Tells the menu a category has moved to a different level - another folder,
   * another sub-view. A level seen before comes back with its cursor where it was;
   * a new one starts at the top. `key` is whatever identifies the level uniquely
   * within the category, such as the folder path.
   */
  enterLevel(categoryId: string, key: string): void {
    this.level.set(categoryId, key);
    // Cheap guard against a very long browsing session growing this without bound.
    // Insertion order makes the oldest entries the ones to drop.
    if (this.selectedIndex.size > LEVEL_MEMORY) {
      const oldest = this.selectedIndex.keys().next();
      if (!oldest.done) this.selectedIndex.delete(oldest.value);
    }
  }
  ...
  private setSelectedIndex(i: number): void {
    ...
    this.selectedIndex.set(this.levelKey(cat.id), clamped);
  }
  ...
  /** Forces the cursor back to the top of the level a category is on. */
  resetSelection(categoryId: string): void {
    this.selectedIndex.set(this.levelKey(categoryId), 0);
  }
